...
class ExperimentConfig:
    ...
    name: str = "short-lora"
    model_path: str = "artifacts/models/Qwen3-0.6B"
    data_path: str = "data/tool_router"
    adapter_path: str = "artifacts/adapters/tool-router-short"
    iterations: int = 40
    num_layers: int = 8
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--adapter-path",
        default="artifacts/adapters/tool-router-short",
    )
...
def validate_config(config: ExperimentConfig) -> None:
    if not config.model_dir.exists():
        raise FileNotFoundError(
            f"Missing {config.model_dir}; "
            "run python -m scripts.download_mlx_model first."
        )
    for split in ("train", "valid", "test"):
        ...
        if not path.exists():
            raise FileNotFoundError(
                f"Missing {path}; "
                "run python -m scripts.prepare_tool_router_data first."
            )
